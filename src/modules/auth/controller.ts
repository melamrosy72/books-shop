...
export const register = async (c: Context) => {
  ...
  if (existingUser) {
    return failureResponse(c, 'User already exists', 409);
  }
  // create user
...
export const login = async (c: Context) => {
  ...
  if (!isValidPassword) return failureResponse(c, 'Invalid Credentials', 401);
...
export const refresh = async (c: Context) => {
  ...
  try {
    const decoded: decodedTokenType = verifyRefreshToken(refreshToken);
    // Check in Redis
    const storedToken = await redis.get(`refresh:${decoded.userId}`);
    // Token not found in Redis
    if (storedToken !== refreshToken) return failureResponse(c, 'Invalid refresh token', 401);
    // Generate new tokens
    ...
    return failureResponse(c, 'Invalid refresh token', 401);
  }
};
