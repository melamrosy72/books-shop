...
import { type RegisterInput, type LoginInput, type UpdateProfileInput, type resetPasswordInput } from "./validation.js"
...
export const resetPassword = async (data: resetPasswordInput) => {
    ...
    if (!existingUser.resetPassOtp) throw new ClientError("User Has Not Requested For Password Reset")
    ...
    await db.update(users)
        ...
        .where(
            and(
                eq(users.email, data.email),
                eq(users.resetPassOtp, data.otp)
            )
        )
