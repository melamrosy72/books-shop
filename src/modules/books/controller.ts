...
import { deleteThumbnail } from '../../utils/ThumbnailFile.js';
...
export const createBook = async (c: Context) => {
  ...
  const book = await booksService.createBook(userId, validatedData, thumbnailFile ?? undefined);
...
export const deleteBook = async (c: Context) => {
  const { bookId } = getBookByIdSchema.parse({ bookId: c.req.param('bookId') });
  ...
  // unlink thumbnail => we can use transaction to make it atomic operation with book deletion
