...
import { getTags, createTag, getAuthors, createAuthor, getCategories, createCategory, deleteBook, getAllBooks, getBookById, getMyBooks, createBook, editBook } from '../src/modules/books/controller.js'
import * as booksService from '../src/modules/books/service.js'
import { bookQuerySchema, createAuthorSchema, createBookSchema, createCategorySchema, createTagSchema, getBookByIdSchema, updateBookSchema } from '../src/modules/books/validation.js'
...
describe('Books Controller - Tags', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(failureResponse).mockImplementation((c, message, status) => c.json({ success: false, error: message }, status))
    })
    ...
    describe('Books Service - Tags', () => {
        describe('createTag', () => {
            ...
            it('should handle database errors during tag creation', async () => {
                ...
                await expect(booksService.createTag(mockTagData)).rejects.toThrow('Database constraint violation')
            })
        })
    ...
    describe('getCategories', () => {
        it('should return all categories successfully', async () => {
            ...
            const mockCategories = [
                { id: 1, name: 'Fiction', description: 'Fiction books', createdAt: new Date(), updatedAt: new Date() },
                { id: 2, name: 'Non-Fiction', description: 'Non-fiction books', createdAt: new Date(), updatedAt: new Date() }
            ]
    ...
    describe('deleteBook', () => {
        it('should delete book successfully', async () => {
            ...
            const mockBook = {
                ...
                ownerId: 1,
                category: { id: 1, name: 'Fiction' },
                tags: [{ id: 1, name: 'tag1' }, { id: 1, name: 'tag2' }],
            }
