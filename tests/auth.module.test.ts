...
import { register, login, logout, resetPassword } from '../src/modules/auth/controller.js'
...
import { registerSchema, loginSchema, resetPasswordSchema } from '../src/modules/auth/validation.js'
import { ClientError } from '../src/utils/errorHandler.js'
...
describe('Auth Controller - Forgot Password', () => {
    ...
    describe('forgotPassword', () => {
        ...
        it('should handle validation errors from forgot password schema', async () => {
            ...
            // Act & Assert
            await expect(forgotPassword(mockContext)).rejects.toThrow('Validation failed: Invalid email format')
        ...
        it('should handle service errors during OTP generation', async () => {
            ...
            // Act & Assert
            await expect(forgotPassword(mockContext)).rejects.toThrow('Database error during OTP generation')
        })
...
describe('Auth Controller - Reset Password', () => {
    ...
    describe('resetPassword', () => {
        ...
        it('should handle validation errors from reset password schema', async () => {
            ...
            const validationError = new Error('Validation failed: Invalid email, OTP must be 6 digits, Password too short')
            ...
            // Act & Assert
            await expect(resetPassword(mockContext)).rejects.toThrow('Validation failed: Invalid email, OTP must be 6 digits, Password too short')
        ...
        it('should handle missing required fields gracefully', async () => {
            ...
            // Act & Assert
            await expect(resetPassword(mockContext)).rejects.toThrow('Email, OTP, and new password are required')
            expect(authService.resetPassword).not.toHaveBeenCalled()
        })
        ...
        it('should handle weak password validation', async () => {
            ...
            // Act & Assert
            await expect(resetPassword(mockContext)).rejects.toThrow('Password must be at least 8 characters')
            expect(authService.resetPassword).not.toHaveBeenCalled()
        })
    })
})
